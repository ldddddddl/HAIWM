...
class ActGenerate(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.xlstm_cfg = xlstm_cfg
        # Calculate modalities_seq_dim, including language if enabled
        self.use_language = getattr(xlstm_cfg, "use_language", False)
        if self.use_language:
            language_dim = getattr(
            ...
            self.modalities_seq_dim = (
                xlstm_cfg.enc_out_dim * 2 + xlstm_cfg.act_model_enc.embedding_dim
            )
        self.z_dim = z_dim
        self.device = device
        ...
        if xlstm_cfg.data_format == "rpy":
            if xlstm_cfg.act_encoder == "xlstm":
                ...
                self.linear6 = nn.Linear(
                    self.modalities_seq_dim, 3, bias=True, device=device
                )
        else:  # == joints
            # linear6 input comes from conv1d(xlstm_out), which has dimension act_model_dnc.embedding_dim
            self.linear6 = nn.Linear(
                xlstm_cfg.act_model_dnc.embedding_dim,
        ...
        self.conv1d = nn.Conv1d(
            xlstm_cfg.act_model_dnc.context_length,
            xlstm_cfg.horizon,
            kernel_size=1,
            padding=0,
        )
...
class SuckerAct(nn.Module):
    def __init__(self, z_dim=128, device="cpu", xlstm_cfg=None) -> None:
        ...
        self.out = nn.Linear(z_dim, 2, bias=True, device=device)
